...
def average(numlis):
    ...
    for i in range(0,len(numlis)):
        ...
        print(numlis[i])
    avg=sum_/len(numlis)
    print("The average is",avg,"its count is", len(numlis))
#%%
"""
End solution
"""
...
cap_list = ['A', 'B', 'C', 'D']
misc_list = ['ball', 3.14, -50, 'university', "course"]
def for_state(slis_1,slis_2,slis_3):
    for state in slis_1:
        print(slis_1)
    for state in slis_2:
        print(state)
...
def menu_it():
    ...
    while True:
        nextnum = input("menu item: ")
        if nextnum == "that's all":
            break
        num_lis.append(nextnum)
    print("You've orderd",num_lis)
#%%
import random  
for i in range(5): 
